...
const DisabledRadioGroup = () => {
  const [, setSelected] = useState(plans[0]);
  return (
    <div>
      <CardBox>
        <div className="flex items-center justify-between mb-2">
          <h4 className="text-lg font-semibold">Disabled Radio Group </h4>
          <MainRadioGroupCode />
        </div>
        <RadioGroup
        ...
        >
          {plans.map((plan) => (
            <Field key={plan} className="flex items-center gap-3 ">
              ...
              <Label className="text-sm text-ld cursor-pointer opacity-50">{plan}</Label>
            </Field>
          ))}
        </RadioGroup>
      </CardBox>
