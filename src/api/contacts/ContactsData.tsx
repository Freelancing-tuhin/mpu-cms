...
mock.onGet('/api/data/contacts/contactsData').reply(() => {
  return [200, JSON.parse(JSON.stringify(ContactList))];
});


// Mock API endpoint to add a new contact
mock.onPost('/api/data/contacts/addContact').reply((config) => {
...
// Define the mock PUT endpoint for updating contact data
mock.onPut('/api/data/contacts/updateContact').reply((config) => {
  const updatedContactData = JSON.parse(config.data);
  ...
  if (updatedContactIndex !== -1) {
    ...
    return [200, ContactList[updatedContactIndex]];
  } else {
    return [404, { message: 'Contact not found' }];
  }
});
