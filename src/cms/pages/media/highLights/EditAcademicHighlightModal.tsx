...
import { updateAcademicHighlight, IAcademicHighlight } from 'src/services/academicHighlights';
...
export default function EditAcademicHighlightModal({ data, isOpen, setIsOpen, refresh }: any) {
  const [form, setForm] = useState<IAcademicHighlight>(data);
  ...
  const handleSubmit = async (e: any) => {
    e.preventDefault();
    try {
      await updateAcademicHighlight(data._id, form);
      refresh();
      setIsOpen(false);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-40 z-50">
      <div className="bg-white p-6 rounded-lg w-full max-w-md relative">
        ...
        <h2 className="text-xl mb-4">Edit Highlight</h2>
